...
Columbo.prototype.build_recent_search_div = function(){
  ...
  self.recent_div = jQuery('<div id=\"_columbo_recent\" style=\"display:non\">')  
  ...
  if(searches.length > 0){
    ...
    jQuery.each(searches.reverse(), function(index, string){
      var redo_search_link = '<a onclick=\"jQuery(\'input#_columbo_search\').val(\''+string+'\'); self.search_highlight();\">'+ string + '</a>';
...
Columbo.prototype.make_interface = function(){
  ...
  jQuery.append(jQuery('<div id="_columbo_close"></div>').click(function () {self.clear_interface();}))
