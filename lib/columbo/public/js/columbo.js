...
Columbo.prototype.dont_highlight_columbo = function(){
  ...
  jQuery.each(self.highlights, function(index, value){
    ...
    jQuery.each(jQuery('.'+highlight), function(index, element){ 
      ...
      // don't hightlight textareas
      if(jQuery(element).parents('textarea').length > 0){
        jQuery(element).parent().unhighlight({className: ''+highlight+''});
      };
      // don't highlight hidden elements
      jQuery.each(jQuery(element).parents(), function(index, parent){
            var style = jQuery(parent).attr('style')
        if(style){
          if(style.replace(/\s/g,'').search(/display:none/i) > -1){
            jQuery(element).removeClass(highlight);
          }    
        }
      });
